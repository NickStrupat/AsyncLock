...
// public sealed class CancellableAsyncLock : IAsyncLock
// {
...
// 		{
// 			// If the task was canceled, put the task back to what it was when we entered.
// 			if (Interlocked.CompareExchange(ref task, prev, next.Task) == next.Task)
// 			{
//
// 			}
// 			_ = prev.ThenSetResultOn(next);
// 			throw;
// 		}
//
// 		try
...
// 			{
// 				// Because no one else entered the lock while we held it, we know that no one is awaiting our 'next' TCS. We can put it back in the "cached TCS" field.
